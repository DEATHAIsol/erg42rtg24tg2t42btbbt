...
const SITE_FEE_ETH = 0.0005 // site fee per trade in ETH (was 0.01 SOL)
...
const [solPrice, setSolPrice] = useState<number>(3000) // fallback ETH price in USD
  ...
  const handleTrade = async () => {
    ...
    try {
      ...
      const required = totalCost + 0.0005 // buffer (was 0.01 SOL)
          ...
          {amount && parseFloat(amount) > 0 && (
            <div className="p-4 bg-terminal-bg rounded-xl border border-terminal-border animate-fade-in">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-terminal-text-muted num">ETH ≈ ${solPrice.toFixed(2)}</span>
              </div>
